<?php require('../../../tools/var/variables.php'); ?>

<head>
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1.0" />
    <link rel="stylesheet" type="text/css" href="<?= $dir ?>/css/styles.css">
</head>
<?=$titulo?>
<?php
    $numeros = array('8','4','10','6','2','1');
?>
<div class="container-two">
    <div class="content ">
        <div class="fila hea">
            <div></div>
            <div>Adición</div>
            <div>Multiplicación</div>
        </div>
        <div class="contentDos aleatorio">
        <div class="fila">
            <div class="image camara">
                <img src="<?= $dir ?>/img/1.png" alt="">
            </div>
            <div class="operacion">
                <div class="numero">1</div>
                <div class="rojo">+</div>
                <div class="numero">1</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="2">
                        <option value="" disabled selected></option>
                        <?php
                            for ($i=0; $i < count($numeros); $i++) { 
                                echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                            }
                        ?>
                    </select>
                </span>
            </div>
            <div class="operacion">
                <div class="numero">2</div>
                <div class="rojo">x</div>
                <div class="numero">1</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="2">
                        <option value="" disabled selected></option>
                        <?php
                                for ($i=0; $i < count($numeros); $i++) { 
                                    echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                                }
                            ?>
                    </select>
                </span>
            </div>
        </div>
        <div class="fila">
            <div class="image">
                <img src="<?= $dir ?>/img/2.png" alt="">
            </div>
            <div class="operacion">
                <div class="numero">2</div>
                <div class="rojo">+</div>
                <div class="numero">2</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="4">
                        <option value="" disabled selected></option>
                        <?php
                            for ($i=0; $i < count($numeros); $i++) { 
                                echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                            }
                        ?>
                    </select>
                </span>
            </div>
            <div class="operacion">
                <div class="numero">2</div>
                <div class="rojo">x</div>
                <div class="numero">2</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="4">
                        <option value="" disabled selected></option>
                        <?php
                                for ($i=0; $i < count($numeros); $i++) { 
                                    echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                                }
                            ?>
                    </select>
                </span>
            </div>
        </div>
        <div class="fila">
            <div class="image">
                <img src="<?= $dir ?>/img/3.png" alt="">
            </div>
            <div class="operacion">
                <div class="numero">3</div>
                <div class="rojo">+</div>
                <div class="numero">3</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="6">
                        <option value="" disabled selected></option>
                        <?php
                            for ($i=0; $i < count($numeros); $i++) { 
                                echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                            }
                        ?>
                    </select>
                </span>
            </div>
            <div class="operacion">
                <div class="numero">2</div>
                <div class="rojo">x</div>
                <div class="numero">3</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="6">
                        <option value="" disabled selected></option>
                        <?php
                                for ($i=0; $i < count($numeros); $i++) { 
                                    echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                                }
                            ?>
                    </select>
                </span>
            </div>
        </div>
        <div class="fila">
            <div class="image">
                <img src="<?= $dir ?>/img/4.png" alt="">
            </div>
            <div class="operacion">
                <div class="numero">4</div>
                <div class="rojo">+</div>
                <div class="numero">4</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="8">
                        <option value="" disabled selected></option>
                        <?php
                            for ($i=0; $i < count($numeros); $i++) { 
                                echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                            }
                        ?>
                    </select>
                </span>
            </div>
            <div class="operacion">
                <div class="numero">2</div>
                <div class="rojo">x</div>
                <div class="numero">4</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="8">
                        <option value="" disabled selected></option>
                        <?php
                                for ($i=0; $i < count($numeros); $i++) { 
                                    echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                                }
                            ?>
                    </select>
                </span>
            </div>
        </div>
        <div class="fila">
            <div class="image">
                <img src="<?= $dir ?>/img/5.png" alt="">
            </div>
            <div class="operacion">
                <div class="numero">5</div>
                <div class="rojo">+</div>
                <div class="numero">5</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="10">
                        <option value="" disabled selected></option>
                        <?php
                            for ($i=0; $i < count($numeros); $i++) { 
                                echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                            }
                        ?>
                    </select>
                </span>
            </div>
            <div class="operacion">
                <div class="numero">2</div>
                <div class="rojo">x</div>
                <div class="numero">5</div>
                <div class="rojo">=</div>
                <span>
                    <select class="slc seleccion" alt="10">
                        <option value="" disabled selected></option>
                        <?php
                                for ($i=0; $i < count($numeros); $i++) { 
                                    echo "<option value='$numeros[$i]'>$numeros[$i]</option>";
                                }
                            ?>
                    </select>
                </span>
            </div>
        </div>
        </div>
    </div>
</div>
<div id="modal1" class="modal">
    <div class="modal-content">
        <h4 class="center">Respuesta correcta</h4>
        <center>
            <img class="materialboxed" width="80%" src="<?= $dir ?>/img/respuesta.png">
        </center>
    </div>
    <div class="modal-footer">
        <a href="#!" class="modal-close waves-effect waves-green btn-flat">Listo!</a>
    </div>
</div>
</body>
<script src="../../../../../js/core.js"></script>
<?php require('../../../tools/botones/botones.php');?>
<script>
$("#next").attr("onclick", "<?=$next?>");
</script>
<script type="text/javascript" src="<?= $dir ?>/js/validate.js"></script>
<script type="text/javascript">
// Formulario - Registrados


function result_tipo_2_4_5() {
    var min = $('#Minutos').text();
    var seg = $('#Segundos').text();
    var milseg = $('#Centesimas').text();
    var tiempo = min + ":" + seg + ":" + milseg;
    r = 0;
    seleccion_lista();

    if (r == 10) {
        console.log(r);
        localStorage.setItem("Nota2-4-5", "2");
        localStorage.setItem("Time2-4-5", tiempo);
        correcto();

    } else {
        console.log(r);
        incorrecto();
        localStorage.setItem("Nota2-4-5", "0");
        localStorage.setItem("Time2-4-5", tiempo);

    }
}
</script>